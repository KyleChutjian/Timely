...
router.get("/enroll/:userId", async (req, res) => {
  ...
  const userCourses = await UserCourse.find(userCoursesFilter, {
    courseId: 1,
    _id: 0,
  }).then((course) => {
    ...
    Course.find({ _id: { $in: courses } }).then((result) => {
      res.send(result);
    });
  });
});
...
router.get("/courses/:courseId/:userId/entry", async (req, res) => {
  console.log(
    `Getting entries in the course with id ${req.params.courseId} and userid ${req.params.userId}`
  );
  try {
    const currentCourse = await Course.findById(req.params.courseId);
    // console.log(currentCourse.name);
